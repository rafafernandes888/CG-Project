...
Configuration parseConfig(std::string filename) {
    std::cerr << "A abrir: " << filename << std::endl;  // ← NOVO (cerr faz flush auto)
    ...
    std::cerr << "Ficheiro aberto!" << std::endl;  // ← NOVO
    ...
    // models → carrega vértices para memória
    // A pasta dos modelos é a mesma pasta do XML
         ...
         model = model->next_sibling("model"))
    {
        ...
        // Carrega os vértices do .3d para memória UMA só vez
